...
conn_template = "../3pt_production/gpd_data/3pt_conn_inputs_man_cfg{cfg}.h5"
...
def process_one(task):
    ...
    for op_name, op_idx in operator_plot_list:
        fig_real_out = (
            f"./ratio_plots_manual_real2pt/{op_name}_realratio_{q_label}_{pf_label}_"
            f"w{w}_flow{tgf}_tsep{tsep_list}_cfgs{len(cfg_list)}.png"
        )
        fig_imag_out = (
            f"./ratio_plots_manual_real2pt/{op_name}_imagratio_{q_label}_{pf_label}_"
            f"w{w}_flow{tgf}_tsep{tsep_list}_cfgs{len(cfg_list)}.png"
        )

        #ratio is a complex jackknife list 
        ratio_out = (
            f"./ratio_jk_manual_real2pt/{op_name}_ratio_jk_{q_label}_{pf_label}_"
...
if __name__ == "__main__":
    os.makedirs("./ratio_plots_manual_real2pt", exist_ok=True)
    os.makedirs("./ratio_jk_manual_real2pt", exist_ok=True)
