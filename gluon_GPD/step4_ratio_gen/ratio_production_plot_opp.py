...
conn_template = "../3pt_production/gpd_data_opp/3pt_opp_conn_inputs_man_cfg{cfg}.h5"
...
operator_plot_list = [
    ...
    ("XYXY", lorentz_list.index((0, 0))),
    ("XTXY", lorentz_list.index((3, 0))),
    ("XYXT", lorentz_list.index((0, 3))),
    ]
...
def process_one(task):
    ...
    for op_name, op_idx in operator_plot_list:
        fig_real_out = (
            f"./ratio_plots_manual_real2pt_opp/{op_name}_realratio_opp_{q_label}_{pf_label}_"
            f"w{w}_flow{tgf}_tsep{tsep_list}_cfgs{len(cfg_list)}.png"
        )
        fig_imag_out = (
            f"./ratio_plots_manual_real2pt_opp/{op_name}_imagratio_opp_{q_label}_{pf_label}_"
            f"w{w}_flow{tgf}_tsep{tsep_list}_cfgs{len(cfg_list)}.png"
        )

        #ratio is a complex jackknife list 
        ratio_out = (
            f"./ratio_jk_manual_real2pt_opp/{op_name}_ratio_jk_opp_{q_label}_{pf_label}_"
        ...
        with h5py.File(ratio_out, "w") as f:
            ...
            f.create_dataset("lorentz", data=np.array(lorentz_list[op_idx], dtype=np.int64))
        ...
        outs.append(fig_real_out)
        outs.append(fig_imag_out)
        outs.append(ratio_out)
    return outs


if __name__ == "__main__":
    os.makedirs("./ratio_plots_manual_real2pt_opp", exist_ok=True)
    os.makedirs("./ratio_jk_manual_real2pt_opp", exist_ok=True)
