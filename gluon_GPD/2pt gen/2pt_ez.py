...
smear_steps = 40
smear_mom = [0, 0, 2.5]
smear_mom_x_str = ("%.3f" % smear_mom[2]).rstrip("0").rstrip(".").replace(".", "p") #the string of momentum smearing info, to be used in saving
...
if latt_info.mpi_rank == 0:
    ...
    parent_path = "/lustre/orion/lgt132/scratch/sicheng/gluon_gpd_benchmark/pion_2pt/pion_2pt_functions"
    with h5py.File(f"{parent_path}/N{smear_steps}_G45_ez/pion_smear{smear_steps}_mom{smear_mom_x_str}_G45_ix{ix}_x{x_src_list[0]}_cfg{start_cfg}.h5", "w") as f:
    ...
    with h5py.File(f"{parent_path}/N{smear_steps}_G5_ez/pion_smear{smear_steps}_mom{smear_mom_x_str}_G5_ix{ix}_x{x_src_list[0]}_cfg{start_cfg}.h5", "w") as f:
