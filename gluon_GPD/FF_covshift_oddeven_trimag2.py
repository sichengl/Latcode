...
wilson_line_list = list(range(0,40))
momentum_list_sink = []
for px in [0, -1, 1]:
    for py in [0, -1, 1]:
        for pz in [0, -1, 1]:
            momentum_list_sink.append([px, py, pz])
#gauge = io.readMILCGauge(f"/lustre/orion/lgt132/world-shared/DATA/MILC/a09m310/gauge/l3296f211b630m0074m037m440e.{204}")
...
for i_cfg,cfg in enumerate(measurement_list):
    ...
    for i_smear, smear in enumerate(smear_list):
        ...
        #deviceSynchronize()
        s = perf_counter()
        before = cp.asarray(gauge.data).copy()
        Qij = gauge.loop(
            [
        ...
        core.getLogger().info(f"Now-before is {cp.max(cp.abs(cp.asarray(gauge.data) - before)).get()}")
        ...
        tr_Fij_after = cp.trace(Fij.data, axis1=-2, axis2=-1) / 3.0
        tr_Fi4_after = cp.trace(Fi4.data, axis1=-2, axis2=-1) / 3.0

        Fij_trace_norm2_after = 3.0 * cp.vdot(tr_Fij_after.ravel(), tr_Fij_after.ravel()).real
        Fi4_trace_norm2_after = 3.0 * cp.vdot(tr_Fi4_after.ravel(), tr_Fi4_after.ravel()).real
        
        core.getLogger().info(f"Fij_trace_norm2={Fij_trace_norm2}")
        core.getLogger().info(f"Fi4_trace_norm2={Fi4_trace_norm2}")
        core.getLogger().info(f"Fij_trace_norm2_after={Fij_trace_norm2_after}")
        core.getLogger().info(f"Fi4_trace_norm2_after={Fi4_trace_norm2_after}")
...
if rank == 0:
    ...
    filename = f"/lustre/orion/lgt132/scratch/sicheng/GPD_calc/FF_data/FF_smear_{smear_len*len(smear_list)}_cfg{start_cfg}.h5"
