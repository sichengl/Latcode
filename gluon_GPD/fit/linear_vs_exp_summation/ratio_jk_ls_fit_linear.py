...
fit_output_dir = os.path.join(base_dir, "data_summation_jk_fit_noexp")
os.makedirs(fit_output_dir, exist_ok=True)
fit_output_path = os.path.join(fit_output_dir, "summation_jk_fit_noexp_dic.p")
...
def summation_fcn(x_values, p):
    ...
    return np.array(y_model, dtype=object)
...
def fit_func(task):
    ...
    for w in w_list:
        for tsep in tsep_list:
            ...
            ratio_tins_summed = np.sum(
                ratio[key][:, tins_skip : tsep + 1 - tins_skip],
                axis=1,
            ).real
...
if __name__ == "__main__":
    ...
    print("saved no-exp summation jk fit results to", fit_output_path, flush=True)
