...
input_dir = os.path.join(base_dir, "data_summation_jk_fit_noexp")
input_path = os.path.join(input_dir, "summation_jk_fit_noexp_dic.p")

ratio_output_dir = os.path.join(base_dir, "data_single_double_ratio_from_summation_jk_noexp")
os.makedirs(ratio_output_dir, exist_ok=True)
ratio_output_path = os.path.join(
    ratio_output_dir,
    f"single_double_ratio_noexp_zref{double_ratio_reference_w}_single_energy_corrected.p",
)

plot_dir = os.path.join(base_dir, "plots_single_double_ratio_from_summation_jk_noexp")
...
for operator_name in operator_list:
    for ratio_name in ["single", "double"]:
        ...
        if ratio_name == "single":
            fig.suptitle(f"{operator_name} no-exp energy-corrected single ratio from summation-jk")
            plot_name = (
                f"{operator_name}_noexp_single_ratio_energy_corrected_ioffe_time_"
                f"zref{double_ratio_reference_w}.png"
            )
        else:
            fig.suptitle(
                f"{operator_name} no-exp double ratio from summation-jk, "
                f"z_ref={double_ratio_reference_w}"
            )
            plot_name = f"{operator_name}_noexp_double_ratio_ioffe_time_zref{double_ratio_reference_w}.png"
